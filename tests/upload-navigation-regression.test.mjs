...
assert.match(modal, />\s*Upload Source\s*</)
assert.match(modal, /font-zt-otez/)
assert.match(modal, /<video[\s\S]*controls className="[^"]*rounded-\[12px\]/)
assert.doesNotMatch(modal, /Source studio|Drop an image or video into the frame\./)
...
assert.match(studioUpload, /sourceUploadOverride \?\? queuedSourceUpload/)
assert.match(studioUpload, /handleComposerSubmit\(\{[\s\S]*editActions: selectedEditActions,[\s\S]*\}, sourceUpload\)/)
