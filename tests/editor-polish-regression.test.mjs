...
function run() {
  ...
  assert.match(editorIndexPage, /`\/editor\/\$\{recentProject\.id\}\$\{tabSuffix\}`/)
  ...
  assert.equal(editorProjectPage.includes("text=\"Project\" delay={0.03}"), false)
  ...
  assert.equal(editorProjectPage.includes('Video processing - ${sourceLabel}'), false)
  ...
  assert.match(inspectorPanel, /lg:col-span-1/)
  ...
  assert.match(sharedLoader, /PrometheusInfinityMark/)
